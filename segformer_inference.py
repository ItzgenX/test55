...
def _load_seg_map(seg_path: Path, size, palette: torch.Tensor, device) -> torch.Tensor:
    ...
    # never predicts it, so this guard is dormant on THIS branch's own real
    # maps. Added anyway for real parity with local_seg.py's guard (field
    # guide Lesson 24) and because the grounded_sam branch's copy of this same
    # function had the identical unguarded palette[ids] lookup and DID crash
    # for real (device-side assert, index out of bounds) the first time
    # inference ran against one of its own precomputed maps -- same shared
    # code shape, same latent risk, ported here before it needs to be found
    # the hard way twice.
...
def _load_seg_ids(seg_path: Path, size, device) -> torch.Tensor:
    """Load a raw class-ID PNG (0..18) as the Long conditioning tensor
    SegIDStructureMapperXL actually consumes -- the class-ID counterpart to
    _load_seg_map (which stays around ONLY for the display panel's colorized
    visualization now, not as the conditioning signal). Mirrors
    local_seg.py's SegJsonDataset._load_seg_ids exactly: same NEAREST resize,
    same +1 shift so 0 is reserved NULL/padding_idx for the mapper's
    embedding table.

    Returns [1, size_h, size_w] Long tensor, values 1..19.
    """
    target_wh = normalize_size(size)   # PIL order: (width, height)
    ids_pil = Image.open(seg_path).convert("L")
    if ids_pil.size != target_wh:
        ids_pil = ids_pil.resize(target_wh, Image.NEAREST)
    ids = torch.from_numpy(np.asarray(ids_pil, dtype=np.int64)).unsqueeze(0)   # [1, H, W], 0..18
    return (ids + 1).to(device)   # -> 1..19, 0 reserved NULL
...
def main(cfg):
    ...
    if size_w % 32 or size_h % 32:
        raise ValueError(
            ...
            f"and seg_map_calculations.py's --width/--height guard."
        )
    ...
    if cfg.local_files_only:
        ...
        # base_model_path/seg_model_path pattern as above.
    ...
    for entry in tqdm(entries, desc="Generating"):
        ...
        with torch.no_grad():
            ...
            # cond_ids: raw class-id Long map (1..19, 0=NULL) -- what actually
            # goes to SegIDStructureMapperXL's embedding stem, matching training.
            ...
            cond_ids   = _load_seg_ids(seg_path, size, device)                 # [1,size_h,size_w] Long
        ...
        if _live_seg_available:
            target_ids = (cond_ids[0] - 1).clamp(min=0)   # [size_h,size_w], undo the +1 NULL shift
        ...
        for k, (pred_pil, raw_pred_pil) in enumerate(zip(preds, raw_preds)):
            ...
            # data/dataset/seg_training_aspect/train.jsonl). Joining output_dir
            # with an absolute rel.parent doesn't nest it, it DISCARDS output_dir
            # entirely (Path.__truediv__ with an absolute right operand returns
            # the right operand as-is, same behavior as os.path.join). That
            # silently wrote every run's debug images into
            # data/custom_dataset/images_seg_map_aspect/ itself -- outside any
            ...
            # for any image processed by both. Same bug, same fix, as
            # grounded_sam_inference.py. Fix: use only the parent folder's NAME
            # (a plain string, never absolute) for light disambiguation, so the
            # join can never escape output_dir no matter what the source
            # manifest's paths look like.
