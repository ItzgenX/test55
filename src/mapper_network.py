...
class SegIDStructureMapperXL(nn.Module):
    """Class-ID conditioning for the segformer branch -- replaces
    FixedStructureMapperXL's RGB-colorize-then-conv path. Input is the raw
    class-id map SegFormer already produces (Long [B,H,W], values 1..19,
    0 reserved as NULL), not a palette-colorized image.

    Why: segmentation classes are categorical, not continuous. Colorizing to
    RGB forces the network to re-derive "this shade of blue = sky" from
    pixel values, and two classes assigned nearby palette colors become
    harder to tell apart than they should be. An embedding table gives each
    class its own learned vector directly -- the standard SPADE/OASIS-style
    semantic-synthesis approach, and what NewStructLoRAConv's gamma/beta
    FiLM modulation is actually built to consume. Same embedding-stem
    pattern as mask2former's SegStructureMapperXL, minus the instance
    channel (SegFormer is semantic-only, no instance ids).

    padding_idx=0 makes the NULL row settle to a true zero vector, which
    matters beyond just "unlabeled": src/model.py's CFG dropout zeroes out
    a whole sample's `c` tensor directly (`c[dropout_mask] = 0`) before the
    mapper ever sees it -- for a Long id map that maps every dropped pixel
    to id 0, so id 0 must decode to "no conditioning" for that zeroing to
    still mean what it's supposed to mean. The dataset loader shifts raw
    0..18 ids to 1..19 for exactly this reason -- see local_seg.py.

    Fully convolutional, same three-stage /8-/16-/32 output as
    FixedStructureMapperXL (matches SDXL's three UNet stages).
    """

    def __init__(self, c_dim: int, num_classes: int = 20, embed_dim: int = 32):
        super().__init__()
        self.c_dim = c_dim
    ...
    def forward(self, c: torch.Tensor, *args, **kwargs):
        """c: Long [B,H,W] or [B,1,H,W] class ids (1..19, 0=NULL). Matches
        the skip_encode=True contract: cond = c straight from the dataset's
        precomputed map, no colorization step in between."""
        c = c.long()
        if c.dim() == 4:
            c = c[:, 0]
