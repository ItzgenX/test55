...
class SD15(ModelBase):
    ...
    def forward(
        ...
        # cs in [-1, 1]
        cond = None  # stays None if cs is empty (no conditioning loras wired yet) -- matches SDXL.forward()
...
class SDXL(ModelBase):
    ...
    def get_conditioning(
        ...
        add_time_ids = self.compute_time_ids(device, dtype, original_size, crops_coords_top_left, target_size)
    ...
    def forward(
        ...
        # add our lora conditioning
        cond = None  # stays None if cs is empty (no conditioning loras wired yet)
        for i, (encoder, dp, mapper, c) in enumerate(zip(self.encoders, self.dps, self.mappers, cs)):
            if cfg_mask is None or cfg_mask[i]:
