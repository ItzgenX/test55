...
class ModelBase(ABC, nn.Module):
    ...
    def soften_conditioning(self, c: torch.Tensor, kernel_size: int) -> torch.Tensor:
        ...
        if not torch.is_floating_point(c):
            # Categorical class-ID conditioning (SegIDStructureMapperXL's raw
            # Long [B,H,W] class-id map, cs=[cond_ids] with skip_encode=True)
            # can't be meaningfully blurred -- averaging class ids 3 and 7
            # doesn't produce "class 5", it produces a value that isn't a
            # valid class at all. F.conv2d below also flatly refuses
            # non-floating-point input (RuntimeError) and a 3D [B,H,W] tensor
            # (expects 4D [B,C,H,W]) -- this call previously reached that
            # crash whenever conditioning_kernel_size > 1 was set for
            # class-ID conditioning (dormant only because every shipped
            # config defaults it to 0). Skip softening for this conditioning
            # type instead: the class-ID mapper already has its own
            # boundary-awareness (see SegIDStructureMapperXL._boundary).
            return c
        if kernel_size % 2 == 0:
