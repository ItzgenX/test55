...
def compute_psnr_ssim(img_a: np.ndarray, img_b: np.ndarray) -> tuple[float, float]:
    """PSNR + SSIM between two uint8 [H, W, 3] RGB arrays of identical shape.

    Used as a per-checkpoint monitoring signal (generated image vs. the real
    validation photo), NOT as a quality benchmark. Read the TREND across
    checkpoints, not the absolute value: the model is following a layout plus
    a text prompt, it is not reconstructing that specific photograph, so the
    absolute numbers sit low and noisy by construction. What makes the trend
    meaningful is holding the scenes AND the seed fixed across checkpoints
    (see segformer_training.py's fixed/fresh split) so the only thing that
    changed between two readings is the weights.

    scikit-image rather than torchmetrics/piq: skimage is already a
    dependency of this environment, the other two are not -- verified, not
    assumed, before choosing it.
    """
    ...
    # Identical images give PSNR = inf, which would poison a mean and render
    # as a broken point in TensorBoard. Clamp to a high-but-finite value.
    if not np.isfinite(psnr):
        psnr = 99.0
    ssim = float(
        structural_similarity(img_a, img_b, channel_axis=2, data_range=255)
    )
    return psnr, ssim
...
def write_training_params_txt(cfg: Any, output_path: Path, device: str, original_cwd: str = "") -> None:
    ...
    added, which is exactly when you would most want it recorded.
    """
