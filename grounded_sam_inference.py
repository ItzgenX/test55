...
def main(cfg):
    ...
    # TWO folders per run (Hydra's own outputs/inference_grounded_sam/runs/
    # <date>/<time>/, holding just .hydra/ logs, plus a second, differently-
    # named outputs/inference/grounded_sam/<timestamp>/ actually holding the
    # images), which is exactly the "creates two folders" bug reported. Hydra's
    # run dir (configs/inference_grounded_sam.yaml's hydra.run.dir, combined
    # with hydra.job.chdir=true) already IS a fresh, unique-per-run folder --
    # no need to build a second one. Same pattern grounded_sam_training.py
    # already uses. cfg.inference.output_dir is no longer read (verified no
